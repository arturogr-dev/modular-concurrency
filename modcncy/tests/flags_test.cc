...
TEST(IntFromEnvTest, NotInEnv) {
  ...
  EXPECT_EQ(modcncy::internal::IntFromEnv("not_in_env", 42), 42);
}

// =============================================================================
TEST(IntFromEnvTest, InvalidInteger) {
  ASSERT_EQ(setenv("IN_ENV", "foo", 1), 0);
  EXPECT_EQ(modcncy::internal::IntFromEnv("in_env", 42), 42);
  unsetenv("IN_ENV");
}
...
TEST(IntFromEnvTest, ValidInteger) {
  ...
  EXPECT_EQ(modcncy::internal::IntFromEnv("in_env", 64), 42);
  unsetenv("IN_ENV");
}
...
TEST(IntFromEnvTest, MaxValidInteger) {
  ...
  EXPECT_EQ(modcncy::internal::IntFromEnv("in_env", 64), 2147483647);
  unsetenv("IN_ENV");
}
